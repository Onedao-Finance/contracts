...
contract ODAOTreasury is Ownable {
    ...
    address public OXY;
    uint public OXYQueue; // Delays change to OXY address
    ...
    /**
        @notice allow approved address to deposit an asset for ODAO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ODAO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ODAO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ODAO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IODAOERC20( ODAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( OXY ).balanceOf( msg.sender ); // Can only borrow against OXY held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ODAO
        @param _amount uint
     */
    function repayDebtWithODAO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IODAOERC20( ODAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ODAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ODAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ODAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ODAO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ODAO decimals
            value_ = _amount.mul( 10 ** IERC20( ODAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.OXY ) { // 9
            OXYQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
