...
contract ODAOBondDepository is Ownable {
    ...
    address public immutable ODAO; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints ODAO when receives principle
    ...
    struct Bond {
        uint payout; // ODAO remaining to be paid
     ...
     *  @param _newDAO address
     */
    function setDAO( address _newDAO) external onlyPolicy() {
        require( _newDAO != address(0) && _newDAO != DAO);
        DAO = _newDAO;
    }
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 ODAO ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) ODAO
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( ODAO ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( ODAO ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( ODAO ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( ODAO ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( ODAO ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to ODAO supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( ODAO ).totalSupply();
    ...
    /**
     *  @notice calculate amount of ODAO available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or ODAO) to the DAO
     *  @return bool
     */
